      ...
      for (let k = 0; k < filesinfo.length; k++) {
        ...
        for (let cnt = 0; cnt < current[k].cmds.length; ++cnt) {
          ...
          if (command === '移動') {
            ...
            element.addEventListener('click', (evt) => { evt.preventDefault(); download(current[k].name, current[k].substring(current[k].name.lastIndexOf('/')+1)); });
          } else {
