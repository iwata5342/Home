      ...
      for (let k = 0; k < filesinfo.length; k++) {
        ...
        for (let cnt = 0; cnt < filesinfo[k].cmds.length; ++cnt) {
          ...
          if (command === '移動') {
            ...
            element.addEventListener('click', (evt) => { evt.preventDefault(); download(currinfo[k].name, currinfo[k].substring(currinfo[k].name.lastIndexOf('/')+1)); });
          } else {
...
function initMkDirButton() {
  ...
  form.addEventListener('submit', event => {
    let elem = $('mkdirtext');
    let val = elem.value;
    if (!form.checkValidity() || val.length > 21 || val.length == 0) {
        event.preventDefault()
